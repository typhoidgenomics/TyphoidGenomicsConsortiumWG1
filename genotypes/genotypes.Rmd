...
# Western Africa - numbers of 3.1.1 across all data
gt3.1.1_WA <- tgc_all %>% filter(Final_genotype=="3.1.1" & Region=="Western Africa") %>% group_by(Country_Origin) %>% summarise(n=n())

tgc_all %>% filter(Region=="Western Africa") %>% group_by(Country_Origin) %>% summarise(N=n()) %>% full_join(gt3.1.1_WA)

# Western Africa - numbers of 2.3.2 across all data
gt2.3.2_WA <- tgc_all %>% filter(Final_genotype=="2.3.2" & Region=="Western Africa") %>% group_by(Country_Origin) %>% summarise(n=n())

tgc_all %>% filter(Region=="Western Africa") %>% group_by(Country_Origin) %>% summarise(N=n()) %>% full_join(gt2.3.2_WA)
...
tgc_meta$Region_orderFig1b <- factor(tgc_meta$Region, levels=c("Southern Asia", "South-eastern Asia", "Eastern Africa", "Southern Africa", "Western Africa", "Central America", "South America",  "Polynesia"))

#select for 2010 - 2020, H58 genotypes plus any genotype >20% prevalent in an individual country
country_facet_39geno_keycountries <- tgc_meta %>%
  mutate(map_genotype = if_else(Final_genotype %in% geno_country20, Final_genotype, "Other"))%>%
  select(Year, map_genotype, Region_orderFig1b, Country_Origin)%>%
  ...
  ylab("")+
  xlab("")+
  facet_wrap(~Region_orderFig1b + country_label, ncol=3) + 
  theme_bw() +
  theme(strip.background = element_rect(fill ="#f0f0f5"), strip.text=element_text(size=8, face="bold"), axis.text.x = element_text(angle = 45, hjust = 1, size = 6, colour = "black"), legend.text = element_text(size=8), legend.key.size=unit(0.25, "cm"), legend.title = element_text(size=8, face="bold")) +
  guides(fill=guide_legend(ncol=1))+
