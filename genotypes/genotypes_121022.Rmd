...
``` {r eval=T}
write.csv(genotypes_by_region, file=paste0("SuppTable5_genotypeFreqs_byRegion_2010-2020_",todays_date,".csv"), row.names=F)
```
...
``` {r eval=T}
write.csv(genotypes_by_country, file=paste0("SuppTable6_genotypeFreqs_byCountry_2010-2020_",todays_date,".csv"), row.names=F)
```
...
pdf(file=paste0("SuppFig2_region_facet_",todays_date,".pdf"),width=7,height=8)
region_facet_39geno
dev.off()

png(paste0("SuppFig2_region_facet_",todays_date,".png"), width=7, height=8, units="in", res=400)
region_facet_39geno
dev.off()
```
...
pdf(file=paste0("SuppFig3_country_facet_exclKey_",todays_date,".pdf"),width=13,height=18)
country_facet_39geno_exclKey
dev.off()

png(file=paste0("SuppFig3_country_facet_exclKey_",todays_date,".png"),width=13,height=18, units="in", res=400)
country_facet_39geno_exclKey
dev.off()
```
...
pdf(file=paste0("Fig1_map_genotypesMainCountries_facet_",todays_date,".pdf"),width=7,height=9)
...
png(file=paste0("Fig1_map_genotypesMainCountries_facet_",todays_date,".png"),width=7,height=9, units="in", res=400)
  genotype_pie_map_countryGeno_nolegend / (country_facet_39geno_keycountries +  guide_area() + plot_layout(widths=c(5,1))) + plot_layout(nrow=2, heights=c(2,3))
dev.off()
```
...
## Fig S7: MDR and CipIR genotypes, countries with ≥50 genomes
``` {r country_facet_MDR_CipIR_geno_figs, eval=T}
pdf(file=paste0("SuppFig7_genotypesMainCountries_MDR_CipIR_",todays_date,".pdf"),width=9,height=12)
country_facet_39geno_keycountries_MDR / country_facet_39geno_keycountries_cipIR
dev.off()

png(file=paste0("SuppFig7_genotypesMainCountries_MDR_CipIR_",todays_date,".png"),width=9,height=12, units="in", res=400)
country_facet_39geno_keycountries_MDR / country_facet_39geno_keycountries_cipIR
dev.off()
```
...
pdf(file=paste0("SuppFig12_country_lab_data_SA_genotypeFreq_estimates_",todays_date,".pdf"),width=8,height=12)
country_lab_data_SA_genotypeFreq_estimates
dev.off()

png(file=paste0("SuppFig12_country_lab_data_SA_genotypeFreq_estimates_",todays_date,".png"),width=8,height=12, units="in", res=400)
country_lab_data_SA_genotypeFreq_estimates
dev.off()
```
...
res_freq_table <- res_data %>%
  ...
  summarize(n=n(),
            ...
            cipIR_sd = sqrt((sumcipIR/n*(1-sumcipIR/n))/n),
...
southAsia_amr_lab <- annual_res_country_lab %>%
  ...
  as.data.frame() %>%
    ...
    filter(AMR %in% c("MDR","cefR","cipR","cipIR")) 
...
pdf(file=paste0("Fig6_annualGeno_annualAMR_by_lab_SouthAsia_",todays_date,".pdf"),width=8,height=12)
...
png(file=paste0("Fig6_annualGeno_annualAMR_by_lab_SouthAsia_",todays_date,".png"),width=8, height=12, units="in", res=400)
(country_facet_lab_SAsia + ggtitle("a) Annual genotype prevalence, by lab")) / (southAsia_amr_lab_plot_combinedEst + ggtitle("b) Annual AMR prevalence, by lab"))
dev.off()
...
country_lab_data_SA_amrFreq <- tgc_from2010 %>% 
  ...
  mutate(`Isolating Lab` = replace(`Isolating Lab`, `Isolating Lab`=="AKU", "Karachi (AKU)")) %>%
  mutate(`Isolating Lab` = replace(`Isolating Lab`, `Isolating Lab`=="JHL", "Lahore (JHL)")) %>%
  select(Country_Origin, Region, `Isolating Lab`, MDR, cipIR, cipR, aziR, CEP, XDR) %>%
  rename(cefR=CEP) %>%
...
SA_amrFreq <- tgc_from2010 %>% 
  select(Country_Origin, MDR, cipIR, cipR, aziR, CEP, XDR) %>%
  rename(cefR=CEP) %>%
  filter(Country_Origin %in% c("Bangladesh", "India", "Nepal", "Pakistan")) %>%
...
pdf(file=paste0("SuppFig13_country_lab_data_SA_amrFreq_estimates_",todays_date,".pdf"),width=8, height=9)
country_lab_data_SA_amrFreq_estimates
dev.off()

png(file=paste0("SuppFig13_country_lab_data_SA_amrFreq_estimates_",todays_date,".png"),width=8, height=9, units="in", res=400)
country_lab_data_SA_amrFreq_estimates
dev.off()
```
...
country_lab_data_Nigeria_amrFreq <- tgc_from2010 %>% 
  ...
  mutate(`Isolating Lab` = replace(`Isolating Lab`, `Isolating Lab`=="IBA", "Ibadan (IBD)")) %>%
  mutate(`Isolating Lab` = replace(`Isolating Lab`, `Isolating Lab`=="ZMC", "Abuja (ZMC)")) %>%
  select(`Isolating Lab`, MDR, cipIR, cipR, aziR, CEP, XDR) %>%
  rename(cefR=CEP) %>%
...
pdf(file=paste0("Fig7_Nigeria_labcomparisons_",todays_date,".pdf"),width=5, height=8)
country_lab_data_Nigeria_genotypeFreq_estimates / country_lab_data_Nigeria_amrFreq_estimates / country_facet_lab_Nigeria
dev.off()

png(file=paste0("Fig7_Nigeria_labcomparisons_",todays_date,".png"),width=5, height=8, units="in", res=400)
country_lab_data_Nigeria_genotypeFreq_estimates / country_lab_data_Nigeria_amrFreq_estimates / country_facet_lab_Nigeria
dev.off()
